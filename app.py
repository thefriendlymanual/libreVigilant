...
def init_db():
    ...
    try:
        ...
        if not conn.execute("SELECT 1 FROM schema_migrations WHERE version='0001'").fetchone():
            ...
            # ---------------------------------------------------------------
            # Create the 7-table schema (uses legacy orgs/user_orgs names;
            # migration 0003 will rename these to projects/user_projects).
            # ---------------------------------------------------------------
            conn.execute("""
            ...
            # ---------------------------------------------------------------
            # Migrate old data → default project + "Initial Assessment".
            # ---------------------------------------------------------------
            if old_statuses or old_notes or old_attachments:
                now = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
                cur = conn.execute(
                    "INSERT INTO orgs (name, slug, created_at) VALUES (?, ?, ?)",
                    ("Default Project", "default", now),
                )
                org_id = cur.lastrowid
                cur = conn.execute(
                    "INSERT INTO assessments (org_id, name, lifecycle, created_at) VALUES (?, ?, 'review', ?)",
        ...
        # -------------------------------------------------------------------
        # Migration 0003 — rename orgs→projects, user_orgs→user_projects;
        #                   drop org_id from users; role org_admin→admin.
        # -------------------------------------------------------------------
        if not conn.execute("SELECT 1 FROM schema_migrations WHERE version='0003'").fetchone():
            conn.execute("PRAGMA foreign_keys = OFF")

            # 1. Rename orgs → projects (no column changes needed).
            conn.execute("ALTER TABLE orgs RENAME TO projects")

            # 2. Recreate users without org_id; update role org_admin→admin.
            conn.execute("""
            ...
            """)
            conn.execute("""
                INSERT INTO users_new (id, email, display_name, password_hash, role, created_at, last_login_at)
                SELECT id, email, display_name, password_hash,
                       CASE WHEN role = 'org_admin' THEN 'admin' ELSE role END,
            ...
            # 3. Recreate assessments with project_id instead of org_id.
            conn.execute("""
            ...
            conn.execute("""
                ...
                SELECT id, org_id, name, lifecycle, start_date, end_date,
                       created_by, finalised_by, finalised_at, created_at
                FROM assessments
            """)
            conn.execute("DROP TABLE assessments")
            conn.execute("ALTER TABLE assessments_new RENAME TO assessments")

            # 4. Create user_projects from user_orgs; update role org_admin→admin.
            conn.execute("""
            ...
            """)
            conn.execute("""
                INSERT INTO user_projects (id, user_id, project_id, role, joined_at)
                SELECT id, user_id, org_id,
                       CASE WHEN role = 'org_admin' THEN 'admin' ELSE role END,
                       joined_at
                FROM user_orgs
            """)
            conn.execute("DROP TABLE user_orgs")
...
def setup():
    # Accessible only when no users exist yet.
    with get_db() as conn:
...
def _user_role_for_project(project_id):
    """Return the current user's role in project_id, or None if not a member."""
    if "user_id" not in session:
        return None
    with get_db() as conn:
        row = conn.execute(
...
def _lifecycle_summary(projects):
    summary = {"draft": 0, "review": 0, "completed": 0}
    for project in projects:
        for a in project["assessments"]:
...
def index():
    ...
    return render_template(
        "home.html",
        projects=projects,
        lifecycle_summary=_lifecycle_summary(projects),
...
@app.route("/projects/<int:project_id>/assessments", methods=["POST"])
@login_required
def create_assessment(project_id):
    ...
    return redirect(url_for("view_assessment", project_id=project_id, asm_id=asm_id))


@app.route("/projects/<int:project_id>/assessments/<int:asm_id>/activate", methods=["POST"])
@login_required
def activate_assessment(project_id, asm_id):
    if _user_role_for_project(project_id) != "admin":
        abort(403)
    _validate_csrf()
    now = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
    with get_db() as conn:
        asm = conn.execute(
            "SELECT id, lifecycle FROM assessments WHERE id = ? AND project_id = ?",
            (asm_id, project_id),
        ).fetchone()
        if not asm:
            abort(404)
...
@app.route("/projects/<int:project_id>/assessments/<int:asm_id>/finalise", methods=["POST"])
@login_required
def finalise_assessment(project_id, asm_id):
    if _user_role_for_project(project_id) != "admin":
        abort(403)
    _validate_csrf()
    now = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
...
@app.route("/projects/<int:project_id>/assessments/<int:asm_id>/delete", methods=["POST"])
@login_required
def delete_assessment(project_id, asm_id):
    if _user_role_for_project(project_id) != "admin":
        abort(403)
    _validate_csrf()
    with get_db() as conn:
...
def create_project():
    ...
    with get_db() as conn:
        ...
        conn.execute(
            ...
            "VALUES (?, ?, 'admin', ?)",
            (session["user_id"], project_id, now),
        )
        conn.commit()
    return redirect(url_for("index"))
...
def delete_project(project_id):
    ...
    remaining = _load_sidebar_projects()
    if remaining:
        return redirect(url_for("index"))
    session.clear()
    return redirect(url_for("login"))
...
@app.route("/projects/<int:project_id>/assessments/<int:asm_id>")
@login_required
def view_assessment(project_id, asm_id):
    if not _user_role_for_project(project_id):
        abort(403)
    with get_db() as conn:
        asm = conn.execute(
            "SELECT id, name, lifecycle, created_at FROM assessments "
...
def _load_assessment_or_abort(asm_id):
    ...
    with get_db() as conn:
        asm = conn.execute(
            "SELECT id, project_id, name, lifecycle FROM assessments WHERE id = ?",
            (asm_id,),
        ).fetchone()
    if not asm:
        abort(404)
    if not _user_role_for_project(asm["project_id"]):
        abort(403)
    return asm
...
def api_get_attachment(att_id):
    with get_db() as conn:
        row = conn.execute(
            "SELECT a.id, a.filename, a.mime_type, a.assessment_id, s.project_id "
    ...
    if not row:
        abort(404)
    if not _user_role_for_project(row["project_id"]):
        abort(403)
